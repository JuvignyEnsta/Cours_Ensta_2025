"""
Le jeu de la vie avec MPI pour séparer le calcul et l'affichage
"""
import pygame as pg
import numpy as np
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        loop = True
        while loop:
            # Réception des données de calcul
            data = comm.recv(source=1)  # Utilisation de recv() au lieu de irecv()
            cells = data['cells']
            appli.draw(cells)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    loop = False
                    comm.send({'stop': True}, dest=1)  # Envoyer un signal d'arrêt à rank 1
    elif rank == 1:
        # Rank 1 : Calcul
        loop = True
        while loop:
            diff = grid.compute_next_iteration()
            # Envoi des données de calcul à rank 0
            comm.send({'cells': grid.cells}, dest=0)
            time.sleep(0.1)  # Ajuster la vitesse de calcul
            # Vérifier si rank 0 a envoyé un signal d'arrêt
